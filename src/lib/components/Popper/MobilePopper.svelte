			...
			{#if type == 'player'}
				<section class="m-metadata">
					<div class="image" style="">
						<img
							decoding="async"
							src={$list.mix[$key].thumbnails[0].url}
							width={$list.mix[$key].thumbnails[0].width}
							height={$list.mix[$key].thumbnails[0].height}
							alt=""
						/>
					</div>
					<div class="metatext">
						<span class="title">{$list.mix[$key].title}</span>
						<span class="artist">
							{$list.mix[$key].artist
								? $list.mix[$key].artist
								: $list.mix[$key].artistInfo.artist[0].text}
						</span>
						<span class="length">
