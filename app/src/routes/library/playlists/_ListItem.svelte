<script lang="ts">
	...
	let DropdownItems = [
		...
		},
		{
			...
			icon: "x",
			action: async () => {
				const promise = await db.deleteSongFromPlaylist(item.videoId, parentPlaylistId);
			...
			<div class="artists secondary">
				{#if item.artistInfo?.artist}
					{#each item?.artistInfo?.artist as subtitle}
						<a class="artist" href={`/artist/${subtitle.browseId}`} sveltekit:prefetch>{subtitle.text}</a>
					{/each}
				{/if}
			</div>
		</div>
