...
import { filter, generateId, iter } from "../../utils";
import type { IDBPlaylistInternal, IDBRequestTarget, IDBPlaylist, Actions, IDBStoreKeys, IDBMessage, Methods } from "./types";
...
export const dbHandler = <Action extends Actions = any, Type extends "favorite" | "playlist" | "playlists" | "favorites" = any, Key extends keyof Methods & `${Action & string}${Capitalize<Type>}` = keyof Methods & `${Action & string}${Capitalize<Type>}` & string, Fn extends Methods[Key] = Methods[Key]>(action: Action, type: Type, ...params: Parameters<Fn>): ReturnType<Fn> => {
