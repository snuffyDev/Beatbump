...
class IDBService {
	...
	async setup() {
		...
		const workerInstance = await ComLink.wrap<typeof import("./worker")>(new Worker(WorkerURL, { name: "IDB", type: 'classic' }));
		this.isReady = true;
		return workerInstance;

	}
	async sendMessage<Action extends Actions = Actions, Type extends "favorite" | "playlist" | "playlists" | "favorites" = any, Key extends keyof Methods & `${Action & string}${Capitalize<Type>}` = keyof Methods & `${Action & string}${Capitalize<Type>}`, Fn extends Methods[Key] = Methods[Key]>(action: Action, type: Type, ...params: Parameters<Fn>): Promise<Awaited<ReturnType<Fn>>['data']> {
		if (!browser) return;
